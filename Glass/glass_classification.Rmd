...
##Brute Force Gradient Boosting without Pre-Processing

A stochastic gradient boosting model is fit to the raw data without pre-processing. The out-of-sample accuracy is estimated using leave-one-out cross-validation. 

The final model consists of **150 trees** with **interaction depth of 2**. 
The **accuracy** is **0.7850467**.
The **concordance(kappa)** is **0.7028226**.

```{r}
DT.brute<-data.table(DT)
DT.brute[,Type:=as.factor(as.character(Type))]

train_controlA<- trainControl(method="LOOCV")

set.seed(123)
gbm.fit.brute1<- train(Type~., data=DT.brute, trControl=train_controlA, method="gbm")
gbm.fit.brute1
...
g_Si<-ggplot(data=DT.omit6,aes(y=Si,x=as.factor(Type)))+xlab('Type')+
    geom_violin(draw_quantiles=c(0.25,0.75))+ geom_jitter(width=.1)

g_Ca<-ggplot(data=DT.omit6,aes(y=Ca,x=as.factor(Type)))+xlab('Type')+
  geom_violin(draw_quantiles=c(0.25,0.75))+ geom_jitter(width=.1)  

g_Fe<-ggplot(data=DT.omit6,aes(y=Fe,x=as.factor(Type)))+xlab('Type')+
...
```{r}
g_Ba<-ggplot(data=DT.omit6,aes(y=Ba,x=as.factor(Type)))+xlab('Type')+
    geom_violin(draw_quantiles=c(0.25,0.75))+geom_jitter(width=.1) # For Group 5 & 7, 7 is somewhat normal (p=0.001) 

g_K<-ggplot(data=DT.omit6,aes(y=K, x=as.factor(Type)))+xlab('Type')+
...
F2:Type 7 Feature: Ba 25% Quantile ~ 0.5, is higher that all other class maxima [Higher Ba ~ Higher P(Type 7)] 
<Use Bootstrap Simulations to Determine Threshold>
