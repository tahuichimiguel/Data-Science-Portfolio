...
sims<-matrix(nrow=10000,ncol=50)
    ...
    samps<- DT.clean[(Type!='7')]$Ba
    for(i in 1:10000){
        sims[i,]<-sample(samps, size=50,replace=TRUE)
        quant[i]<-quantile(sims[i,],probs=0.95)
    }
    F2_Thresh<-mean(quant)
    DT.feat[,F2:=ifelse(Ba>=F2_Thresh,(Ba-F2_Thresh)^2,0)]
...
```{r}
lda.feat<-lda(Type~.,data = DT.feat)
lda.feat
lda.feat$svd

mat.lda.feat<-as.matrix(DT.feat[,-c('Type'),with=FALSE])
mat.lda.feat<- mat.lda.feat %*% lda.feat$scaling
DF.lda.feat<-data.frame(mat.lda.feat,DT.clean$Type)
names(DF.lda.feat)<-c('LD1','LD2','LD3','LD4','Type')

ggplot(data=DF.lda, aes(x=LD2,fill=as.factor(Type)))+geom_density(alpha=0.25) # Type 5 & 7

ggplot(data=DF.lda.feat, aes(x=LD1,fill=as.factor(Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.lda.feat, aes(x=LD2,fill=as.factor(Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.lda.feat, aes(x=LD3,fill=as.factor(Type)))+geom_density(alpha=0.25)
ggplot(data=DF.lda.feat, aes(x=LD4,fill=as.factor(Type)))+geom_density(alpha=0.25) 

```

##PCA w/ Engineered Features
```{r}
PCA.feat<-prcomp(x=DT.feat[,-c('Type'),with=FALSE])

mat.pca.feat<-as.matrix(DT.feat[,-c('Type'),with=FALSE])
mat.pca.feat<- mat.pca.feat %*% PCA.feat$rotation
DF.pca.feat<-data.frame(mat.pca.feat,DT.feat$Type)

summary(PCA.feat)

ggplot(data=DF.pca.feat, aes(x=PC1,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.pca.feat, aes(x=PC2,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.pca.feat, aes(x=PC3,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25)
ggplot(data=DF.pca.feat, aes(x=PC4,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.pca.feat, aes(x=PC5,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.pca.feat, aes(x=PC6,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.pca.feat, aes(x=PC7,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
ggplot(data=DF.pca.feat, aes(x=PC8,fill=as.factor(DT.feat.Type)))+geom_density(alpha=0.25) 
 
ggplot(data=DF.pca.feat, aes(x=PC1+PC3,y=PC2,colour=as.factor(DT.feat.Type)))+geom_point()

```

PCA & LDA with engineered features can distinguish glass types 5 & 7 very well. Will have to use 2 Step Process:
1) Identify if it is of type 5, 6, or 7
2) If it is not type 5, 6, or 7...use a separate analysis for types 1, 2, and 3
