...
##Brute Force GBM
```{r}
DT.clean[,Type:=as.factor(as.character(Type))]

train_controlA<- trainControl(method="LOOCV")
train_controlB<- trainControl(method="cv", number=(5))

set.seed(123)
gbm.fit.brute1<- train(Type~., data=DT.clean, trControl=train_controlA, method="gbm")
gbm.fit.brute2<- train(Type~., data=DT.clean, trControl=train_controlB, method="gbm")

#gbm.fit.brute1
confusionMatrix(predict(gbm.fit.brute1,DT.clean),DT.clean$Type)

#gbm.fit.brute2
#confusionMatrix(predict(gbm.fit.brute2,DT.clean),DT.clean$Type)
```
