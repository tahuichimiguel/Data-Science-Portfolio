...
path='/Users/Mikey/Documents/Case-Studies/US Police Shooting Fatalities'
setwd(path)
...
##Victim
    ...
    #Age Distribution
    g.guard.age<-ggplot(data = clean.guard.dt,aes(x=age,fill=as.factor(year)))+
        geom_histogram(bins=35)+ facet_grid(.~year)+scale_fill_discrete(guide=FALSE)+
        xlab('Age')+ylab('Fatalities')+    theme(text = element_text(size = 12))
...
##Situation
    ...
    #Location (States)
    g.guard.state<-ggplot(data=clean.guard.dt, aes(x=reorder(state, table(state)[state]))) + 
        geom_bar(fill='red')+coord_flip()+
        xlab('State')+ylab('Fatalities')+
        theme(text = element_text(size = 10))
    
    #Armed
    g.guard.armed<-ggplot(data=clean.guard.dt, aes(x=reorder(armed, table(armed)[armed]))) + 
        geom_bar(fill='red')+coord_flip()+
...
##Victim
    ...
    g.wp.age<-ggplot(data = clean.wp.dt, aes(age,fill=as.factor(year)))+
      geom_histogram(bins=35)+ facet_grid(.~year) +xlab('Age')+ylab('Fatalities')+
