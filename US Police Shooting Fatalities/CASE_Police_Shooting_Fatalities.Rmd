...
#Interaction Heatmap of Guardian Data

Use Male Gunshot Data - Subset on Gender & Classification
* Variables of Interest
        + Ordinal: age, month
        + Factors: armed(8) race(8)
...
age.bins.N <-length(unique(clean.guard.dt[age>=0,.(age=(age-age%%5)),]$age))
month.N <- length(levels(as.factor(clean.guard.dt[age>=0]$month)))
armed.N <- length(levels(clean.guard.dt[age>=0]$armed))
race.N<-length(levels(clean.guard.dt[age>=0]$race))
