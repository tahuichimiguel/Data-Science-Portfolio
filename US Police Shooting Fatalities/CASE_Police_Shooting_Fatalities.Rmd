...
for (i in 1:total.levels) {
    ...
    for (j in 1:total.levels) {
        ...
        # mat.heat.norm[j,i]<- nrow(clean.guard.dt[((age>=0) & gender=='Male' & classification =='Gunshot'  & 
        #                                               eval(cond1) & eval(cond2))])/
        #                         (nrow(clean.guard.dt[((age>=0) & eval(cond1))])+1)          
...
heatmap(mat.heat+1,labCol=pred.names,labRow = pred.names,
        Rowv = NULL,Colv = NA,scale = 'column',na.rm = FALSE,
